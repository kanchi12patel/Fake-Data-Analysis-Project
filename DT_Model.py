...
ctree=DecisionTreeClassifier(random_state=579)
ctree.fit(xtrain,ytrain)
ytest=ctree.predict(xtest)

xtrainx,xtrainy,ytrainx,ytrainy=train_test_split(xtrain,ytrain,test_size=0.2,random_state=579)
ctree1=DecisionTreeClassifier(random_state=579)
ctree1.fit(xtrainx,ytrainx)
pred=ctree1.predict(xtrainy)
...
result.to_csv("DT_ytest.csv",index=False)
